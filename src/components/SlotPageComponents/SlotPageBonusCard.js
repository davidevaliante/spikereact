...
const SlotPageBonusCard = (props) => {
    ...
    const { bonus, image, rating, name } = props.bonus
    return (
        <div>
            <Card color='red' onClick={(event) => handleClick()}>
                <Card.Content>
                    <Card.Header style={{ marginBottom: '0.8rem' }}> <Image src={getImageLinkFromName('bonus', name, 'medium')} size='small' circular /></Card.Header>
